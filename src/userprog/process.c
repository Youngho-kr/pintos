...
process_execute (const char *file_name) 
{
  ...
  /* Sync for load() 
     Before exec end, caller have to know exec is success or fail
  */
  sema_down(&cur->sema_exec);

  struct thread *child = searchChild(tid);
  if(child == NULL)
    return -1;
...
{
  struct thread *child = searchChild(child_tid);
...
process_exit (void)
{
  ...
  if (pd != NULL) 
    ...
    /* ?? */
    /* After executing file is end, writing to file is allow */
    ...
    // printf("process exit1\n");
    // sema_up(&cur->sema_wait);
    // printf("process exit2\n");
    // sema_down(&cur->sema_exit);
    // printf("process exit3\n");
}
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  for(int i = argc-1; i >= 0; i--) {
    ...
    *esp -= strSize;
    ...
    memcpy(*esp, argv[i], strSize);
  }
  /* word-align */
  if(wordAlign % 4 != 0) {
    *esp -= (4 - wordAlign % 4);
    memset(*esp, 0, 4 - wordAlign % 4);
  }
  /* argv[] */
  *esp -= sizeof(char *);
